...
import threading
import ctypes
from sha256 import sha256_verify
from diffie_hellman import get_value_to_send, generate_psk
...
if __name__ == "__main__":
    ...
    if not sha256_verify(str(psk), checksum):
    ...
    def send_messages(client_socket):
        while True:
            try:
                message = input("\033[1;32mDigite a mensagem que será enviada ao servidor (ou digite 'exit' para sair): \033[0m")
                ...
                if message:
                    ...
                    # Salvar mensagem encriptada em arquivo
                    with open(f"{username}_messages.txt", "a") as f:
                        f.write(f"Mensagem Enviada: {message}\n")
                        f.write(f"Mensagem Encriptada: {encrypted_message}\n")
            except Exception as e:
    ...
    client.close()
    print("Conexão encerrada. Cliente finalizado.\n")
